  ...
  if (payload.type) {
    ...
    switch (payload.type) {
      ...
      case "device-provisioned":
        ...
        pgConnetion
          .query(`SELECT * FROM SetGetDeviceName ($1::varchar)`, [
            authSet.identity_data.mac,
          ])
          .then((responseSetGetDeviceName) => {
            if (
              responseSetGetDeviceName.rows &&
              responseSetGetDeviceName.rows.length > 0
            ) {
              let deviceName = responseSetGetDeviceName.rows[0].Name;
              axios
                .put(
                  `${menderURL}/api/management/v1/inventory/devices/${payload.data.id}/tags`,
                  [{ name: "name", value: deviceName }],
                  {
                    headers: {
                ...
                .then((res) => {
                  Log(
                    `Name for device updated > ${payload.data.id} (${deviceName})`,
                    MODULE
                  );
                })
                .catch((err) => {
